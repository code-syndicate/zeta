...
var { Debit3, Notification3, Credit3 } = require("../models/transactions");
...
async function history(req, res) {
  ...
  const debits = await Debit3.find(options)
    ...
    .limit(20)
    .exec();
  const credits = await Credit3.find(options)
    .populate("issuer")
...
const signInPOST = [
  ...
  body("accountId").custom(async (inputValue, { req }) => {
    const IdExists = await Customer3.exists({ accountId: inputValue });

    if (!IdExists) {
      throw Error(`No account is associated with ID '${inputValue}'`);
    }

    const user = await Customer3.findOne({ accountId: req.body.accountId })
      .lean()
      .exec();
...
const signUpPOST = [
  ...
  body("email").custom(async function (inputValue) {
    const emailExists = await Customer3.exists({ email: inputValue });
